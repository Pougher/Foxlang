    ...
    while (i < this->m_tokens.size()) {
        ...
        switch (tok_type) {
            case TK_LET: {
                ...
                // check if the variable is being defined naked under a
                // namespace
                if (this->m_bracestack.curly_brace.size() > 0) {
                    if (this->m_bracestack.curly_brace[
            ...
            case TK_IDENTIFIER: {
                ...
                if (i + 2 < this->m_tokens.size()) {
                    if (this->m_tokens[i + 1].get_type() == TK_COLON &&
                        this->m_tokens[i + 2].get_type() == TK_COLON) {
                        std::string namespace_name = "";
                        uint64_t iter = i;

                        const std::string begin_scope_id = \
                            std::get<std::string>(tok[0]);
                        Object_t* scope_root = this->m_scope.contains_node_up(
                            begin_scope_id);
                        ...
                        this->m_scope.push();
                        this->m_scope.set(scope_root); // center the tree
                                                       // around `scope_root`

                        namespace_name += begin_scope_id;

                        while (iter + 3 < this->m_tokens.size()) {
                            if (this->m_tokens[iter + 1].get_type() == TK_COLON
                                && this->m_tokens[iter + 2].get_type() ==
                                TK_COLON && this->m_tokens[iter + 3].get_type()
                                == TK_IDENTIFIER) {
                                const std::string child_name = \
                                    std::get<std::string>(
                                        this->m_tokens[iter + 3][0]
                                    );
                        ...
                        this->c_program += namespace_name + \
                                           this->m_scope.get()->metadata[0];
                        this->m_scope.pop();
            ...
            case TK_FN: {
                ...
                for (unsigned long iter = i; i < this->m_tokens.size();
                    iter++) {
            ...
            case TK_LCPAREN: {
                if (brace_skip) {
                    ...
                    std::vector<std::string> collected = \
                        this->m_scope.collect();
                    std::string scope_name = Common::brace_scope_name(
                        collected);
            ...
            case TK_RCPAREN: {
                ...
                if (this->m_bracestack.curly_brace_top() ==
                    Brace_t::NAMESPACE) {
            ...
            case TK_MINUS: {
                ...
                if (i + 2 < this->m_tokens.size()) {
                    if (this->m_tokens[i + 1].get_type() == TK_GREATER &&
                        this->m_tokens[i + 2].get_type() == TK_IDENTIFIER) {
                        // ignore identifier existence
                        this->c_program += "->" + std::get<std::string>(
                            this->m_tokens[i + 2][0]);
                        i += 3;
                        continue;
                    }
            ...
            case TK_STRUCT: {
                ...
                this->m_scope.add_child(Object_t(name, "struct",
                    OBJ_TYPE_STRUCT));
            ...
            case TK_UNION: {
                ...
                this->m_scope.add_child(
                    Object_t(name, "union", OBJ_TYPE_UNION));
                this->m_scope.traverse(name);
                this->m_indent_level++;
            ...
            case TK_NAMESPACE: {
                if (i + 2 < this->m_tokens.size()) {
                    ...
                    this->m_scope.add_child(Object_t(ident, "none",
                        ObjectType_t::OBJ_TYPE_NAMESPACE));
                    this->m_scope.traverse(ident);

                    namespace_list.push_back(ident);
                    this->m_bracestack.curly_brace.push_back(
                        Brace_t::NAMESPACE);
                    this->m_indent_level ++;
                    i += 2;
                } else {
        ...
        bool* success)
{
    ...
    for (auto el : expected) {
        if (el == this->m_tokens[*i].get_type()) {
            ...
            std::cerr << ERROR_SYNTAX << " on line " << this->m_line << \
                std::endl;
