    ...
    for (unsigned long i = 0; i < this->m_file.length(); i++) {
        ...
        if (IS_DELIM(this->m_file[i])) {
            if (this->m_toks != "" && !this->m_state["any_active"] && \
                FOX_TYPES.find(this->remove_asterisks(this->m_toks)) == FOX_TYPES.end()) {
                ...
                this->m_tokens.push_back(make_token<std::string>(TokenType_t::TK_IDENTIFIER, tk_copy));
                this->m_ftoks = this->m_file[i];
                if (IS_DELIM(delim)) {
                    this->m_toks = delim;
                    this->m_ftoks = delim;
...
void Lexer_t::keyword_mapcheck(const std::map<const std::string, Token_t>& kw, char n)
{
